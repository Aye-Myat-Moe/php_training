<template>
  <v-card class="mx-auto" max-width="344">
    ...
    <v-form ref="form" v-model="valid">
      ...
      <v-card-actions class="create-post-action">
        <v-dialog v-model="dialog" persistent max-width="600">
          <template v-slot:activator="{ on }">
            <v-btn :disabled="!valid" large color="success" class="mr-4" v-on="on">Create</v-btn>
            <v-btn color="error" large class="mr-4" @click="resetForm">Clear</v-btn>
          </template>
          <v-card>
            <v-card-title class="headline">Create Post Confirmation</v-card-title>
            <v-card-text>
              <v-row>
                <v-col cols="4">
                  <span>Title :</span>
